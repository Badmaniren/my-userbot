...
def ask_gemini(user_id, sender_name, user_text, card_text):
    ...
    system_instruction = (
        ...
        "- Если собеседник ржет ('хаха', 'ахах', 'лол') — отвечай иронично, подкалывай ('Че ржешь?', 'Губу закатай', 'Смешно тебе?'). НЕ МОЛЧИ!\n"
        "- Если наезжает или тупит — отвечай резко, с сарказмом, отшивай.\n\n"
        "СТИЛЬ РЕЧИ:\n"
        "- Используешь междометия: 'Ну', 'Бля', 'Крч', 'Тэкс'.\n"
        "- Эмоциональность: 'Ахуеть', 'Пиздец', 'Балдежно', 'Кайф'.\n"
        "- Отвечаешь строго ПО ДЕЛУ на последнее сообщение.\n\n"
        "ПРАВИЛА:\n"
        "1. ВЫДАЙ ТОЛЬКО ОДНУ ФРАЗУ НА РУССКОМ. БЕЗ ПРЕДИСЛОВИЙ И АНГЛИЙСКОГО.\n"
        "2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО МОЛЧАТЬ. Обязательно дай ответ."
    )
    ...
    try:
        ...
        if 'candidates' in response and len(response['candidates']) > 0:
            ...
            # Если нейросеть сгенерила пустой ответ на смех/эмодзи — подставляем пацанский фоллбэк
            if not ans:
                if any(w in user_text.lower() for w in ['хах', 'ахах', 'лол', 'lol', 'гы']):
                    ans = "Чё ржёшь?"
                else:
                    ans = "Мда."
    ...
    fallback = "Чё ржёшь?" if any(w in user_text.lower() for w in ['хах', 'ахах', 'лол']) else "Занят, позже перетрем."
...
async def main():
    ...
    await load_db()
    print("\n==========================================")
    print(" ЮЗЕРБОТ СТАРТОВАЛ (v3.6: ФИКС СМЕХА И ФОЛЛБЭК) ")
    print("==========================================\n")
    await idle()
    await app.stop()
