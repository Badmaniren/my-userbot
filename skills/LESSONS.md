...
ов проходит успешно, а не отдаётся именно тело — это похоже на задержку публикации логов у GitHub, а не на нехватку прав токена. Ручного вмешательства это не требует: этот раунд пр
...
- Последняя ошибка перед фиксом: return super().coordinate_expansion(url, timeout)
- Статус: ПРОВАЛЕН Унгой, передан на эскалацию
