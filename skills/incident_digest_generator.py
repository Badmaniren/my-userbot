...
def start_new(digest_name: str, incidents_stream, output_path: str):
    ...
    try:
        ...
        try:
            ...
            if isinstance(processed_incidents, list):
                if len(processed_incidents) == 1 and isinstance(processed_incidents[0], dict):
                    context = processed_incidents[0]
                else:
                    context = {"incidents": processed_incidents}
            elif isinstance(processed_incidents, dict):
                context = processed_incidents
            else:
                context = {"data": processed_incidents}
            rendered_payload = engine.render_text(digest_name, context)
