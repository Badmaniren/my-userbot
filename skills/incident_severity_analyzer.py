...
def start_new(payload):
    ...
    if "error_code" in payload:
        try:
            if callable(incident_aggregator):
                incident_aggregator(payload)
            elif hasattr(incident_aggregator, "aggregate_incidents"):
                incident_aggregator.aggregate_incidents("incident_severity_analyzer", RuntimeError("payload error"), "")
        except Exception:
            raise
