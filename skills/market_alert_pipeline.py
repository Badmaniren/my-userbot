...
class MarketAlertPipeline:
    ...
    def process_and_check(self, symbol, url, threshold):
        ...
        active_threshold = threshold if threshold is not None else db_threshold

        if active_threshold is not None and current_price > active_threshold:
            alert_data = {
                "symbol": symbol,
                "url": url,
                "price": current_price,
    ...
    def process_alert_check(self, symbol, url, threshold_price):
        ...
        alert_triggered = threshold_price is not None and current_price > threshold_price
