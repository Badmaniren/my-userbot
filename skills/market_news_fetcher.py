...
def extract_tickers(text: str) -> List[str]:
    ...
    # 1. Шаблоны для совпадений вида $AAPL или NASDAQ:AAPL
    cashtag_matches = re.findall(r'\$([A-Z0-9]{2,8})', text)
    for t in cashtag_matches:
        tickers.add(t)

    exchange_matches = re.findall(r'\b[A-Z]{2,10}:([A-Z0-9]{2,8})\b', text)
    for t in exchange_matches:
        tickers.add(t)
