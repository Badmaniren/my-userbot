...
class MarketPortfolioAuditComplianceHub:
    ...
    def run_compliance_export(self, export_path):
        if hasattr(self.audit_exporter, "storage_file") and self.audit_exporter.storage_file and not os.path.exists(self.audit_exporter.storage_file):
            try:
                with open(self.audit_exporter.storage_file, "w", encoding="utf-8") as f:
                    f.write("{}")
            except Exception:
                pass
