...
class SystemAuditLogExporter:
    ...
    def export_stream(self, target_url: str, log_stream) -> dict:
        try:
            ...
            raw_str = stream_content.decode("utf-8", errors="replace") if isinstance(stream_content, bytes) else str(stream_content)
            try:
                json_data = json.loads(raw_str)
            except Exception:
                json_data = {"raw": raw_str}
...
def export_system_audit_log(data: dict) -> dict:
    ...
    if "telemetry" in data:
        export_data["telemetry"] = data["telemetry"]
