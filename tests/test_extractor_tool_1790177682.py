...
class TestExtractorTool1790177682(unittest.TestCase):
    ...
    def test_extract_invalid_input(self) -> None:
        ...
        for item in invalid_inputs:
            with self.subTest(item=item):
                res = self.tool.extract_metadata_from_markup(item)
                self.assertEqual(res, {})

    def test_extract_xml_entries(self) -> None:
        markup = f'<root><metadata><entry key="{self.meta_name}" value="{self.meta_content}" /></metadata></root>'
