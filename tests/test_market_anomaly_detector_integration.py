...
class TestMarketAnomalyDetectorIntegration(unittest.TestCase):
    ...
    def test_detect_insider_activity_spike(self):
        # Генерируем случайный тикер и уникальный ID сессии для исключения кэширования
        test_ticker = f"STK-{uuid.uuid4().hex[:6].upper()}"
        correlation_id = str(uuid.uuid4())

        # Формируем исторический базис (нормальное состояние рынка)
        ...
        # Генерируем аномальный всплеск (имитация инсайда)
        ...
        # Проверка реального изменения в db_storage (интеграция с БД)
        ...
        # Проверка отправки события в alert_sink (интеграция с шиной событий)
    ...
    def test_ignore_normal_market_fluctuations(self):
        ...
        # Нормальные данные без резких отклонений
        ...
        # Проверяем, что в хранилище аномалий запись не появилась
