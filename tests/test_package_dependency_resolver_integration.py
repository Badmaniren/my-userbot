...
class TestPackageDependencyResolverIntegration(unittest.TestCase):
    ...
    def test_dependency_resolution_and_recovery_flow(self):
        ...
        real_pkg = "requests"
        versions = self.client.get_release_versions(real_pkg)
        self.assertIsInstance(versions, list)

        metadata = self.client.get_package_metadata(real_pkg)
        self.assertIsInstance(metadata, dict)

        dependencies = self.client.get_dependencies(real_pkg)
        ...
        pipeline_result = self.pipeline.run_pipeline(
            ...
            exception=exc,
            traceback_str=tb,
            context={"run_id": self.random_str, "incident_id": incident_id}
        )
